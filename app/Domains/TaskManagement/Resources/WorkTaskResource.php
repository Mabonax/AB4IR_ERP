...
class WorkTaskResource extends JsonResource
{
    public function toArray($request): array
    {
        ...
        return [
            ...
            'marketing_operations_count' => (int) ($this->marketing_requests_count ?? 0) + (int) ($this->marketing_deliverables_count ?? 0),
            'marketing_operations' => $this->whenLoaded('marketingRequests', fn () => $this->marketingRequests->map(fn ($request) => [
                'id' => $request->id,
                'title' => $request->title,
                'status' => $request->status,
                'priority' => $request->priority,
                'due_date' => $request->due_date?->format('Y-m-d'),
                'deliverables_count' => $request->deliverables_count ?? $request->deliverables()->count(),
            ])->values()),
            'marketing_deliverables_count' => $this->marketing_deliverables_count ?? 0,
            'marketing_deliverables' => $this->whenLoaded('marketingDeliverables', fn () => $this->marketingDeliverables->map(fn ($deliverable) => [
                'id' => $deliverable->id,
